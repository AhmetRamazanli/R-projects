...
churn <- read_csv("C:/Users/Hp/Desktop/R/week_11/Churn_Modelling (1).csv")
churn %>% skim()
churn %>% glimpse()


# Remove unneeded columns.
churn <- churn %>% select(-CustomerId,-RowNumber, -Surname)
...
churn.num <- churn %>% select_if(is.numeric) %>% select(Exited, everything())
...
churn.iv <- churn %>% select(Exited,ivars)

churn.iv %>% dim()

bins <- churn.iv %>% woebin("Exited")
